...
public class OutsiderBedSensor extends Sensor<OutsiderEntity> {
    ...
    protected void update(ServerWorld worldIn, OutsiderEntity entityIn) {
        BedWorldSaveData     bedWorldSaveData = BedWorldSaveData.get(worldIn);
        Collection<BlockPos> bedPoses     = bedWorldSaveData.entityBedsMap.get(entityIn.getUniqueID());
        Optional<BlockPos> nearestPrivateBed = bedPoses.stream().filter(worldIn::isBlockLoaded).filter(t -> {
            Path path = entityIn.getNavigator().getPathToPos(t, 1);
            return path != null;
        }).min(Comparator.comparingDouble(t -> entityIn.getDistanceSq(new Vec3d(t))));

        if(nearestPrivateBed.isPresent())
            entityIn.getBrain().setMemory(MemoryModuleType.NEAREST_BED,nearestPrivateBed);
        else {
            this.checkTimes = 0;
