...
public class OutsiderMovementController extends MovementController {
    ...
    public void tick() {
        ...
        if (this.action == MovementController.Action.STRAFE) {
            ...
            if (deltaY > (double) this.mob.stepHeight && deltaX * deltaX + deltaZ * deltaZ < (double) Math.max(1.0F,
                                                                                                               this.mob.getWidth()) || !voxelshape.isEmpty() && this.mob.getPosY() < voxelshape.getEnd(
